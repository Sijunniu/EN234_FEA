...
!=========================== ABAQUS format user element subroutine ===================
      ...
      SUBROUTINE UEL_b(RHS,AMATRX,SVARS,ENERGY,NDOFEL,NRHS,NSVARS,
    ...
    !
      double precision  ::  strain(2)                         ! Strain vector contains [e11, e22, e33, 2e12, 2e13, 2e23]
      ...
      double precision  ::  E, xnu, h, ww                     ! Material properties
      double precision  ::  utemp(22)                         ! Temporary displacement (for incompatible mode elements)
      double precision  ::  xi0(2)                            ! All zero xi to calculate eta0 (det0)
      double precision  ::  dNdxi0(9,2)                       ! 2D shape function for det0
      double precision  ::  dxdxi0(2,2)                       ! Derivative of spatial coords wrt normalized coords for det0
      double precision  ::  T(8,6)
      double precision  ::  sin1, sin2, cos1, cos2, L
      double precision  ::  slavecoord(2,4)                   ! Slave coord
      double precision  ::  ehat(2,2)                         ! Laminar coord
      double precision  ::  R(2,3)                            ! Rotation tensor
      double precision  ::  MM(2,6)
      double precision  ::  xi12(2,5)
      double precision  ::  TT, VV, M
    ...
    !     PROPS(2)         Poisson's ratio
     ...
     1       + (coords(2,2)-coords(2,1))**2.d0)
      sin1 = (coords(2,2)-coords(2,1))/L
      sin2 = (coords(2,2)-coords(2,1))/L
      cos1 = (coords(1,2)-coords(1,1))/L
      cos2 = (coords(1,2)-coords(1,1))/L
      
      slavecoord(1,1) = coords(1,1) + h*sin1/2.d0
      slavecoord(2,1) = coords(2,1) - h*cos1/2.d0
      slavecoord(1,2) = coords(1,2) + h*sin2/2.d0
      slavecoord(2,2) = coords(2,2) - h*cos2/2.d0
      slavecoord(1,3) = coords(1,2) - h*sin2/2.d0
      slavecoord(2,3) = coords(2,2) + h*cos2/2.d0
      slavecoord(1,4) = coords(1,1) - h*sin1/2.d0
      slavecoord(2,4) = coords(2,1) + h*cos1/2.d0
      
      n_points = 5

    ! Write your code for a 2D element below
      
      call abq_UEL_1D_integrationpoints(n_points, 4, xi1, w)
      ...
      T(7,1) = 1.d0
      T(8,2) = 1.d0
      T(1,3) = coords(2,1) - slavecoord(2,1)
      T(2,3) = -coords(1,1) + slavecoord(1,1)
      T(3,6) = coords(2,2) - slavecoord(2,2)
      T(4,6) = coords(1,2) - slavecoord(1,2)
      T(5,6) = coords(2,2) - slavecoord(2,3)
      T(6,6) = coords(1,2) - slavecoord(1,3)
      T(7,3) = coords(2,1) - slavecoord(2,4)
      T(8,3) = -coords(1,1) + slavecoord(1,4)
      
      D(1,1) = E
      D(2,2) = E/(1.d0+xnu)/2.d0
      
      ENERGY(1:8) = 0.d0
      
        do kint = 1, n_points
            call abq_UEL_2D_shapefunctions(xi12(1:2,kint),4,N,dNdxi)
            dxdxi = matmul(slavecoord(1:2,1:4),dNdxi(1:4,1:2))
            ...
            dxdxi1 = dxdxi(1:2,1)/norm2(dxdxi(1:2,1))
            dxdxi2 = dxdxi(1:2,2)/norm2(dxdxi(1:2,2))
            ehat(1:2,1) = dxdxi1
            ehat(1:2,2) = dxdxi2
            R = 0.d0
            R(1,1) = ehat(1,1)*ehat(1,1)
            R(1,2) = ehat(2,1)*ehat(2,1)
            R(1,3) = ehat(1,1)*ehat(2,1)
            R(2,1) = 2*ehat(1,1)*ehat(1,2)
            R(2,2) = 2*ehat(2,1)*ehat(2,2)
            R(2,3) = ehat(1,1)*ehat(2,2)+ehat(1,2)*ehat(2,1)
            
            MM = matmul(R,matmul(B(1:3,1:8),T))
            strain = matmul(MM,U(1:6))

            stress = matmul(D,strain)
            
            RHS(1:6,1) = RHS(1:6,1)
     1           - matmul(transpose(MM),stress)*w(kint)*ww*L*h/2.d0

            AMATRX(1:6,1:6) = AMATRX(1:6,1:6)
     1        + matmul(transpose(MM),matmul(D,MM))*w(kint)*ww*L*h/2.d0

            TT = TT + ww*h*stress(1)/2.d0*w(kint)
            VV = VV + ww*h*stress(2)/2.d0*w(kint)
            M = M + ww*h*h*xi12(2,kint)*stress(1)/4.d0*w(kint)
    !        ENERGY(2) = ENERGY(2)
    ...
    !
      ...
      subroutine abq_2d_dtmn(Ain,determinant)
    
        implicit none
    
        double precision, intent(in)    :: Ain(2,2)
        double precision, intent(out)   :: determinant
    
        double precision :: A(2,2)
    
        A(1:2,1:2) = Ain(1:2,1:2)
        determinant = A(1,1)*A(2,2)-A(1,2)*A(2,1) 
        
      end subroutine abq_2d_dtmn