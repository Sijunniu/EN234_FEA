...
!=========================== ABAQUS format user element subroutine ===================
      ...
      SUBROUTINE UEL_3DD(RHS,AMATRX,SVARS,ENERGY,NDOFEL,NRHS,NSVARS,
    ...
    !
    ! Local Variables
      integer      :: i,j,k,n_points,kint, nfacenodes, ipoin
    ...
    !
      ...
      double precision  ::  mu,Kbulk,G1,G2,G3,G4              ! Material properties
      double precision  ::  F(3,3)                            ! Deformation gradient
      double precision  ::  q_matrix(3,3)                     ! q component in matrix form
      double precision  ::  q(9)                              ! Normal stress
      double precision  ::  Sigma(3,3)                        ! Material stress
      double precision  ::  H(6,9)                            ! Some matrix
      double precision  ::  kab(20,20)                        ! Geometric stiffness
      double precision  ::  G(6,6)                            ! Elastic moduli
      double precision  ::  Y(60,60)
      double precision  ::  stress_mat(3,3)                   ! Stress matrix form
      double precision  ::  jay                               ! det(F)
    ...
    !     --  Loop over integration points
      do kint = 1, n_points
        ...
        F(1,1:3) = matmul(U(1:3*NNODE-2:3),dNdx)     ! Assemble deformation gradient tensor
        F(2,1:3) = matmul(U(2:3*NNODE-1:3),dNdx)
        F(3,1:3) = matmul(U(3:3*NNODE:3),dNdx)
        forall(k = 1:3) F(k,k) = F(k,k)+1
        
        call Sigma_D(mu,Kbulk,G,F,Sigma,D,jay)        ! Use separate subroutine to calculate Sigma and D
        
        q_matrix = matmul(Sigma,transpose(F))
        q(1) = q_matrix(1,1)
        q(2) = q_matrix(2,2)
        q(3) = q_matrix(3,3)
        q(4) = q_matrix(2,1)
        ...
        kab = matmul(dNdx,matmul(Sigma,transpose(dNdx)))
        Y(1:3*NNODE-2:3,1:3*NNODE-2:3) = kab(1:NNODE,1:NNODE)
        Y(2:3*NNODE-1:3,2:3*NNODE-1:3) = kab(1:NNODE,1:NNODE)
        Y(3:3*NNODE:3,3:3*NNODE:3)     = kab(1:NNODE,1:NNODE)

        
        stress_mat = matmul(F,matmul(Sigma,transpose(F)))/jay
        stress(1) = stress_mat(1,1)
        stress(2) = stress_mat(2,2)
        stress(3) = stress_mat(3,3)
        stress(4) = stress_mat(1,3)
        stress(5) = stress_mat(1,3)
        stress(6) = stress_mat(2,3)
     ...
     2     matmul(D,matmul(H,B(1:9,1:3*NNODE)))))
     3      +Y(1:3*NNODE,1:3*NNODE))*w(kint)*determinant
      ...
      END SUBROUTINE UEL_3DD

      subroutine Sigma_D(mu,Kbulk,G,F,Sigma,D,J)  
      
      implicit none
      
      double precision, intent(in) :: mu
      double precision, intent(in) :: Kbulk
      double precision, intent(in) :: F(3,3)
      double precision, intent(in) :: G(6,6)
      double precision, intent(out) :: Sigma(3,3)
      double precision, intent(out) :: D(6,6)
      double precision, intent(out) :: J
      
      double precision :: C_mat(3,3)          ! Cauchy-Green
      double precision :: C(6)
      double precision :: Cbar(6)
      double precision :: Cminus_mat(3,3)
      double precision :: Cminus(6)
      double precision :: Finv(3,3) 
      double precision :: P(6)
      double precision :: Cstar(6)
      double precision :: Cbs(6)
      double precision :: Q,K
      double precision :: Omega(6,6)
      double precision :: Iden(6)
      double precision :: Sigma_vec(6)
      integer :: i,l
      
      C_mat = matmul(transpose(F),F)
      C(1) = C_mat(1,1)
      C(2) = C_mat(2,2)
      C(3) = C_mat(3,3)
      C(4) = C_mat(1,2)
      C(5) = C_mat(1,3)
      C(6) = C_mat(2,3)
      call abq_UEL_invert3d(F,Finv,J)
      call abq_UEL_invert3d(C_mat,Cminus_mat,K)
      Cbar = C/(J**(2.d0/3))
      Cminus(1) = Cminus_mat(1,1)
      Cminus(2) = Cminus_mat(2,2)
      Cminus(3) = Cminus_mat(3,3)
      Cminus(4) = Cminus_mat(1,2)
      Cminus(5) = Cminus_mat(1,3)
      Cminus(6) = Cminus_mat(2,3)
      Cstar(1) = C_mat(1,1)
      Cstar(2) = C_mat(2,2)
      Cstar(3) = C_mat(3,3)
      Cstar(4) = 2.d0*C_mat(1,2)
      Cstar(5) = 2.d0*C_mat(1,3)
      Cstar(6) = 2.d0*C_mat(2,3)
      Cbs = Cstar/(J**(2.d0/3))
      ...
      Q = 1/4.d0*dot_product((Cbar-Iden),matmul(G,(Cbar-Iden)))
      P = 1/(2*(J**(2.d0/3)))*(matmul(G,(Cbs-Iden))-1.d0/3*
     1      (dot_product(Cstar,matmul(G,(Cbar-Iden))))*Cminus)
      Sigma_vec = mu*exp(Q)*P+Kbulk*J*(J-1)*Cminus
      Sigma(1,1) = Sigma_vec(1)
      Sigma(2,2) = Sigma_vec(2)
      Sigma(3,3) = Sigma_vec(3)
      Sigma(1,2) = Sigma_vec(4)
      Sigma(1,3) = Sigma_vec(5)
      Sigma(2,3) = Sigma_vec(6)
      Sigma(2,1) = Sigma_vec(4)
      Sigma(3,1) = Sigma_vec(5)
      Sigma(3,2) = Sigma_vec(6)
      
      Omega(1,1) = Cminus(1)*Cminus(1)
      Omega(2,2) = Cminus(2)*Cminus(2)
      Omega(3,3) = Cminus(3)*Cminus(3)
      Omega(4,4) = (Cminus(1)*Cminus(2)+Cminus(4)*Cminus(4))/2
      Omega(5,5) = (Cminus(1)*Cminus(3)+Cminus(5)*Cminus(5))/2
      Omega(6,6) = (Cminus(2)*Cminus(3)+Cminus(6)*Cminus(6))/2
      Omega(1,2) = Cminus(4)*Cminus(4)
      Omega(1,3) = Cminus(5)*Cminus(5)
      Omega(1,4) = Cminus(1)*Cminus(4)
      Omega(1,5) = Cminus(1)*Cminus(5)
      Omega(1,6) = Cminus(4)*Cminus(5)
      Omega(2,3) = Cminus(6)*Cminus(6)
      Omega(2,4) = Cminus(2)*Cminus(4)
      Omega(2,5) = Cminus(4)*Cminus(6)
      Omega(2,6) = Cminus(2)*Cminus(6)
      Omega(3,4) = Cminus(5)*Cminus(6)
      Omega(3,5) = Cminus(3)*Cminus(5)
      Omega(3,6) = Cminus(3)*Cminus(6)
      Omega(4,5) = (Cminus(1)*Cminus(6)+Cminus(4)*Cminus(5))/2
      Omega(4,6) = (Cminus(4)*Cminus(6)+Cminus(2)*Cminus(5))/2
      Omega(5,6) = (Cminus(3)*Cminus(4)+Cminus(5)*Cminus(6))/2
      do i = 2, 6
          do l = 1, i-1
              Omega(i,l) = Omega(l,i)
          end do
      ...
      D = mu*exp(Q)*(1/(J**(4.d0/3.d0))*(G-1.d0/3*(matmul(G,
     1   spread(Cstar,dim=2,ncopies=6)*spread(Cminus,dim=1,ncopies=6))+
     2   spread(Cminus,dim=2,ncopies=6)*spread(matmul(G,Cstar),dim=1,
     3   ncopies=6))-1.d0/3*(J**(2.d0/3.d0))*(dot_product(Cstar,
     4   matmul(G,(Cbs-Iden))))*Omega+1.d0/9*dot_product(Cstar,
     5   matmul(G,Cstar))*spread(Cminus,dim=2,ncopies=6)*
     6   spread(Cminus,dim=1,ncopies=6)))+mu*exp(Q)*
     7   (2*spread(P,dim=2,ncopies=6)*spread((P-1.d0/3*Cminus),
     8   dim=1,ncopies=6)-1/(3*(J**(2.d0/3.d0)))*
     9   spread(Cminus,dim=2,ncopies=6)*spread(matmul(G,(Cbs-Iden)),
     a   dim=1,ncopies=6))+Kbulk*J*((2*J-1)*
     b   spread(Cminus,dim=2,ncopies=6)*spread(Cminus,dim=1,ncopies=6)+
     c   2*(J-1)*Omega)
      
      end subroutine Sigma_D
