...
!=========================== ABAQUS format user element subroutine ===================
      ...
      SUBROUTINE UEL_pf(RHS,AMATRX,SVARS,ENERGY,NDOFEL,NRHS,NSVARS,
    ...
    !
      ...
      double precision  ::  dxdxibar(2,2)                       
      double precision  ::  dxidxbar(2,2)
    ...
    !
      ...
      double precision  ::  E, nu, Omega, ww, kappa, Dr, theta! Material properties
      double precision  ::  qvec(9)                           ! q vector
      double precision  ::  sol(9)                            ! Solution vector
      double precision  ::  dsol(9)                           
      double precision  ::  c, fc, dfdc, dc, mu               ! Concentration
      double precision  ::  De(3,3)                           ! Element D matrix
      double precision  ::  Dold(4,4)                         ! De 4 by 4
      double precision  ::  Iden(4)                           ! Identity vector
...
!      n_points = 4
    ! Write your code for a 2D element below
      ...
      D(4,4) = 1.d0     ! constant part of D matrix
      D(6,8) = -kappa
      D(7,9) = -kappa
      D(8,6) = theta*Dr
      D(9,7) = theta*Dr
      
      Dold(1,1) = E/((1.d0+nu)*(1.d0-2*nu))*(1.d0-nu)
      Dold(2,2) = E/((1.d0+nu)*(1.d0-2*nu))*(1.d0-nu)
      Dold(3,3) = E/((1.d0+nu)*(1.d0-2*nu))*(1.d0-nu)
      Dold(4,4) = E/((1.d0+nu)*2)
      Dold(1,2) = E/((1.d0+nu)*(1.d0-2*nu))*nu
      Dold(2,1) = E/((1.d0+nu)*(1.d0-2*nu))*nu
      Dold(1,3) = E/((1.d0+nu)*(1.d0-2*nu))*nu
      Dold(3,1) = E/((1.d0+nu)*(1.d0-2*nu))*nu
      Dold(2,3) = E/((1.d0+nu)*(1.d0-2*nu))*nu
      Dold(3,2) = E/((1.d0+nu)*(1.d0-2*nu))*nu
      
      De(1:2,1:2) = Dold(1:2,1:2)
      De(3,3) = E/((1.d0+nu)*2)

!      c = 0.5d0+0.01d0*sin(15*)
      ...
      do kint = 1, n_points
          ...
          call abq_2d_dtmn(dxdxi,determinant)
          dNdx(1:NNODE,1:2) = matmul(dNdxi(1:NNODE,1:2),dxidx)
          
          call abq_UEL_2D_shapefunctions(xi(1:2,kint),4,Nbar,dNbardxi)   ! calling 4 noded shapefunction to calculate B
          dxdxibar = matmul(coords(1:2,1:4),dNbardxi(1:4,1:2))
          call abq_inverse_LU(dxdxibar,dxidxbar,2)
          dNbardx(1:4,1:2) = matmul(dNbardxi(1:4,1:2),dxidxbar)
          ...
          stress = matmul(Dold,strain-Iden*Omega*c/3)
          fc = 2*ww*c*(c-1.d0)*(2*c-1.d0)
          dfdc = ww*(12*c*c-12*c+2.d0)
          
          D(1:3,1:3) = De(1:3,1:3)
          D(1,5) = -Omega/3.d0*(Dold(1,1)+Dold(1,2)+Dold(1,3))
          D(2,5) = -Omega/3.d0*(Dold(2,1)+Dold(2,2)+Dold(2,3))
          D(4,1) = -Omega/3.d0*(Dold(1,1)+Dold(1,2)+Dold(1,3))
          D(4,2) = -Omega/3.d0*(Dold(2,1)+Dold(2,2)+Dold(2,3))
          D(4,5) = -dfdc+Omega*Omega/9.d0*(Dold(1,1)+Dold(1,2)+Dold(1,3)
     1         +Dold(2,1)+Dold(2,2)+Dold(2,3)
     2         +Dold(3,1)+Dold(3,2)+Dold(3,3))
          D(5,5) = 1.d0/DTIME
          
          qvec(1) = stress(1)
          qvec(2) = stress(2)
          qvec(3) = stress(4)
          qvec(4) = mu-fc-Omega*(stress(1)+stress(2)+stress(3))/3.d0
          qvec(5) = dc/DTIME
          qvec(6) = -kappa*sol(8)
          qvec(7) = -kappa*sol(9)
          qvec(8) = Dr*(sol(6)+(theta-1.d0)*dsol(6))
          qvec(9) = Dr*(sol(7)+(theta-1.d0)*dsol(7))
          
          
          RHS(1:24,1) = RHS(1:24,1)
     1        - matmul(transpose(B),qvec)*w(kint)*determinant

          AMATRX(1:24,1:24) = AMATRX(1:24,1:24)
     1        + matmul(transpose(B),matmul(D,B))*w(kint)*determinant
